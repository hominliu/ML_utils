...
class BasicTuner(ABC):
    def __init__(
        ...
        """ML Model hyper-parameters tuner

        Args:
            model_type: type of the model
            training_params: training parameters that will be used in both tuning
                process and final model training process
            hyper_params_tuning_values: hyper-parameters with list of values that will
                be trained (tuned) over the tuning process
            fixed_hyper_params: hyper-parameters with fixed value over the tuning
                process
            find_min_matrix: indicator for whether to find the minimum in the matrices
                from the tuning process for the best hyper-parameters set
            check_overfitting: check if potential overfitting presented in best
                iteration's cross-validation results if True
            verbose_best_params: print out the best hyper-parameters if True
        """
    ...
    def _cross_validate(self, train_x, train_y, params: dict):
        ...
        (within cv) set based on provided hyper-parameters
        """
    ...
    def _train_final_model(
        ...
        """use best hyper-parameters set get from tuning to train the final model"""
        raise NotImplementedError
    ...
    def _load_best_hyper_params_post_tuning(
        ...
        """Load best hyper-parameter set based on model cross-validation results.
        Note:
    ...
    @time_process(process_name="Hyper-parameters Tuning")
    def _model_hyper_params_tuning(self, train_X, train_y):
        """Tune model hyper-parameters through cross-validations
        ...
        print(f"Start tuning hyper-parameters: ({len(self.hyper_params_sets)} sets)")
    ...
    def _set_hyper_params_combination(
        self, params_to_grid, base_params: dict = None
    ) -> list:
        """Load combinations of provided hyper-parameters into iterable list

        Args:
            params_to_grid: dictionary with key as hyper-parameter and value as a
                list of hyper-parameter values, a dictionary containing hyper-parameters
                that will be tuned over cross-validation
            base_params: dictionary with key as hyper-parameter and value as hyper-
                parameter value, a dictionary containing the hyper-parameters that will
                be constant over cross-validation
        Return:
            a list of dictionaries with unique combination of hyper-parameters
        """
        if not base_params:
            base_params = {}
    ...
    def tune(self, train_X, train_y, test_X, test_y, **kwargs):
        ...
        print(f"{self.model_type} hyper-parameters tuning end.")
